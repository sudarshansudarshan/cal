...
export const fetchProgress = createAsyncThunk(
  ...
  async ({ courseInstanceId, sectionItemId }, { dispatch }) => {
    const response = await dispatch(anotherApiService.endpoints.fetchSectionItemsProgress.initiate({ courseInstanceId, sectionItemId }));
    if (response.error) {
...
const progressSlice = createSlice({
  ...
  reducers: {
    ...
    clearProgress(state, action) {
      ...
      console.log('Ratattatattatattataaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',state[itemKey]);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProgress.rejected, (state, action) => {
        ...
        const { courseInstanceId, sectionItemId } = action.meta.arg;
        const progressKey = `${courseInstanceId}-${sectionItemId}`;
        console.error(`Error fetching progress for key: ${progressKey}`, action.error);
      });
  }
});
