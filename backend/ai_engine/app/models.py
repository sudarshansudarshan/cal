...
def generate_questions_from_prompt(text: str, user_api_key: str, 
                                   n_questions: int, q_model: str) -> str:
    n = 4  # Number of questions to generate
    task_description_simple = """
        ...
        }""" + f"\n Your input will be a transcript, and you will generate {n} questions based on its content in this exact format."
    ...
    # if True:
    #     task_description = task_description_simple
    # else:
...
def process_video(url, user_api_key, timestamps, segment_wise_q_no, segment_wise_q_model):
    ...
    if transcript == []:
        ...
        timestamps = sorted(timestamps)
        timestamps.append(audio_duration+1)  # Ensure last timestamp is the duration of the audio
        ...
        for i, segment in enumerate(segments):
            questions_response = generate_questions_from_prompt(segment["text"], user_api_key, 
                                                                segment_wise_q_no[i], segment_wise_q_model[i])
            question_data = parse_llama_json(questions_response)
            print(question_data)

            if "case_study" in question_data:
                case_study_text = question_data.pop("case_study")  # Remove "case_study" key and get its text
